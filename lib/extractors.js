...
var extractors = {
    ...
    store: function (file, destination, zip) {
        ...
        if (file.uncompressedSize === 0) {
            ...
            writer = pipePromise.bind(null, input, destination, file.mode);
        }
    ...
    deflate: function (file, destination, zip) {
        ...
        .then(function () {
            if (file._maxSize <= zip.chunkSize) {
                ...
                return pipePromise(inflater, destination, file.mode);
            }
        })
        .then(function () {
...
var pipePromise = function (input, destination, mode) {
    var deferred = Q.defer();
    var output = fs.createWriteStream(destination, { mode: mode });
